...
## 📊 7. 스텟 / 상태 (Status) 상세 화면 구성 (`assets/sprites/menu_status/`)
*💡 골드와 클릭수는 상단 헤더 카운터에 상시 표시되므로 스텟창에서 제외되었습니다. 허기/행복은 0부터 25까지 총 26단계의 직관적인 256x256 도트 게이지 스프라이트로 커스텀할 수 있습니다.*
...
| [ ] | **스텟 고정 라벨 레이어** | `status_labels_layer.png` | 256x256 px | '레벨', '허기', '행복' 고정 텍스트/도트 틀 레이어 |
