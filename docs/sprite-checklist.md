...
이 문서는 사용자가 직접 제작하실 다마고치 펫, 아이템, UI, **기계 단말기(Machine Case) & 상점(Shop) 에셋**의 **필수 제작 목록, 파일명 규격, 추천 규격 및 제작 진행 상황**을 추적하는 체크리스트입니다. 새로운 기능이나 아이템이 추가될 때마다 개발자가 자율적으로 업데이트합니다.
...
## 🕹️ 2. 기계 본체 & 챔버(Machine & Chamber) 에셋 체크리스트
...
## 🛒 3. 상점 (Shop) & 아이템 에셋 체크리스트 (NEW)

| 제작 상태 | 아이템명 | 권장 파일명 | 추천 규격 | 설명 |
| :---: | :--- | :--- | :--- | :--- |
| [ ] | **상점 버튼 아이콘** | `ui_icon_shop.png` | 32x32 px (투명 PNG) | 하단 덱 상점(🛒) 버튼 아이콘 |
| [ ] | **기본 사과 (Apple)** | `food_apple.png` | 32x32 px (투명 PNG) | 허기 +20 회복 (기본 음식) |
| [ ] | **고급 고기 (Meat)** | `food_meat.png` | 32x32 px (투명 PNG) | 허기 +50, 행복도 +15 회복 |
| [ ] | **신선한 생선 (Fish)** | `food_fish.png` | 32x32 px (투명 PNG) | 허기 +35, 경험치 +20 획득 |
| [ ] | **치즈 조각 (Cheese)** | `food_cheese.png` | 32x32 px (투명 PNG) | 허기 +25, 행복도 +10 회복 |
